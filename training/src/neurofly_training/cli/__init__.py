"""The neurofly command: one entry point for building, training and exporting.
    ...
    neurofly detect-train data/objects --out runs/det1 --backend dfine   # then --detect runs/det1
    neurofly detect-list | detect-install yolo               # backends, licences, installing
...
COMMANDS = {
    ...
    "detect-list": ("detect_backends", "the detector backends, installed or not, with licences"),
    ...
    "body-serve": ("body_serve", "host the MuJoCo body and drive it over JSON (WebSocket or "
                                 "stdio), poses pushed to viewers"),
    "body-replay": ("body_replay", "play joint trajectories through the body without physics: "
                                   "the tripod gait, or a real fly's walking"),
